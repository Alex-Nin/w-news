...
const NewPage = () => {
  ...
  const renderNewArticles = (articles) => {
    return articles.map(article => 
      <div className='news-page-item' id={contentWidth}>
        <img alt='news-new-item' src={article.image}/>
    ...
    return details.map(detail =>
      <>
        <div className="trending-item">
            <Link to="../trending"><h3 className="trending-item-title">{detail.title}</h3></Link>
            <p>{detail.info}</p>
        </div>
      </>
  ...
  return( 
    <>
      <h1 id={contentWidth}>Latest Stories</h1>
      {renderNewArticles(newNewsDetails)}

      <div className="trending-side" id={style}>
        ...
        {renderSideLayout(trendingArticleDetails)}
      </div>
