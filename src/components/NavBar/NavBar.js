...
const NavBar = () => {
  ...
  const [style, setStyle] = useState("toggle-close");

  const toggleMenu = () => {
    if(style === "toggle-close") {
      setStyle("toggle-open");
    }else {
      setStyle("toggle-close");
    }
  }
  ...
  return (
    <div className="header">
      ...
      <nav className='navigation' id={style}>
      <svg onClick={toggleMenu} id='close-menu' width="32" height="31" xmlns="http://www.w3.org/2000/svg"><g fill="#00001A" fill-rule="evenodd"><path d="m2.919.297 28.284 28.284-2.122 2.122L.797 2.419z"/><path d="M.797 28.581 29.081.297l2.122 2.122L2.919 30.703z"/></g></svg>
        <ul>
          <li onClick={toggleMenu}><Link to="/w-news">Home</Link></li>
          <li onClick={toggleMenu}><Link to="/w-new/new">New</Link></li>
          <li onClick={toggleMenu}><Link to="/w-new/trending">Trending</Link></li>
        </ul>
      </nav>
      <svg onClick={toggleMenu} id='open-menu' width="40" height="17" xmlns="http://www.w3.org/2000/svg"><g fill="#00001A" fill-rule="evenodd"><path d="M0 0h40v3H0zM0 7h40v3H0zM0 14h40v3H0z"/><path d="M0 0h40v3H0z"/></g></svg>
    </div>
  )
}
